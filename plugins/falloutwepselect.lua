...
PLUGIN.author = "vin"
PLUGIN.desc = "Chnages the weapon selector to fit the Fallout theme."
...
if (CLIENT) then
	...
	local getFoColor = fo.ui.GetHUDColor()
	function PLUGIN:HUDPaint()
		...
		for k, v in SortedPairs(LocalPlayer():GetWeapons()) do
			...
			if (k == self.lastSlot) then
				local foColor = fo.ui.GetHUDColor()
				color.r = foColor.r
				color.g = foColor.g
				color.b = foColor.b
			end
			...
			nut.util.drawText(string.upper(v:GetPrintName()), x, y, color)
